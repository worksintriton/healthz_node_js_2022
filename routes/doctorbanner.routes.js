...
router.post('/create', async function(req, res) {
  try{
        ...
        await doctorbannerModel.create({
...
router.post('/filter_date', function (req, res) {
        doctorbannerModel.find({}, function (err, StateList) {
          var final_Date = [];
...
router.get('/deletes', function (req, res) {
      doctorbannerModel.remove({}, function (err, user) {
...
router.post('/getlist_id', function (req, res) {
        doctorbannerModel.find({Person_id:req.body.Person_id}, function (err, StateList) {
...
router.get('/getlist', function (req, res) {
        doctorbannerModel.find({}, function (err, Functiondetails) {
...
router.get('/mobile/getlist', function (req, res) {
        doctorbannerModel.find({show_status:true}, function (err, Functiondetails) {
          let a = {
...
router.post('/edit', function (req, res) {
        doctorbannerModel.findByIdAndUpdate(req.body._id, req.body, {new: true}, function (err, UpdatedDetails) {
...
doctorbannerModel.findByIdAndRemove(req.body._id, function (err, user) {
          if (err) return res.json({Status:"Failed",Message:"Internal Server Error", Data : {},Code:500});
